...
async function readTenantKey() {
  try {
    const key = (await readFile2(tenantKeyPath(), "utf8")).trim();
    return key || void 0;
  } catch {
    return void 0;
  }
}
async function writeTenantKey(key) {
  await ensureDir(tenantsDir());
  await writeFile2(tenantKeyPath(), `${key}
`, { mode: 384 });
  await chmod(tenantKeyPath(), 384).catch(() => {
  });
}
...
async function provision(workspace, log) {
  ...
  async function workspaceViews(currentKey, apiUrl, owner) {
    ...
    for (const [key, inbox] of Object.entries(state.workspaces)) {
      ...
      rows.set(key, {
        ...
        address,
        planned_address: resolved ? legacy ? planned({ key, title: inbox.title || existing?.title || key, path: inbox.path || existing?.path || "" }) : null : existing?.planned_address ?? null,
